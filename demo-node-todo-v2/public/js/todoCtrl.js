...
todomvc.controller('EnvironmentCtrl', function($scope, $log, $http){
  $log.debug("In environment controller");

  $http.get('/env').then(function(response) {
    $scope.Values = response.data;
  });
});

// Define the Binding controller
todomvc.controller('BindingCtrl', function($scope, $log, $http){
  $log.debug("In binding controller");
  $scope.Bound = true;

  $scope.$watch('activeDb', function() {
    var db = ($scope.activeDb === 'Postgres') ? 'pg' : 'ms';
    ...
    var status = $http.get("/" + db + "_conn_status").then(function(resp, headers){
      var value = resp.data;
...
todomvc.controller('InstanceCtrl', function($scope, $log, $http){
  $log.debug("In instance controller");
  var env = $http.get("/instance").then(function(resp, headers){
    var values = resp.data;
    console.log(values);
